...
class TestAuthentication:
    ...
    def test_create_without_auth(self, unauthenticated_project_service: ProjectService):
        ...
        unauthenticated_project_service.check_response_status(expected_status=401)

    def test_get_project_without_auth(self, unauthenticated_project_service: ProjectService, new_project_id):
        """[GET -] Запрос проекта без токена авторизации (ожидаем 401)."""

        unauthenticated_project_service.get_project_by_id(new_project_id)
        unauthenticated_project_service.check_response_status(expected_status=401)

    def test__update_without_auth(self, unauthenticated_project_service: ProjectService, new_project_id):
        """[PUT -] Попытка изменить проект без токена авторизации (ожидаем 401)."""
        update_data = {"title": UPDATED_TITLE}
        unauthenticated_project_service.update_project(new_project_id, update_data)
        unauthenticated_project_service.check_response_status(expected_status=401)
...
class TestMethodSecurity:
    @pytest.mark.negative
    def test_disallowed_delete_on_creation_endpoint(self, project_service: ProjectService):
    ...
    def test_disallowed_patch_on_project_id(self, project_service: ProjectService, new_project_id):
