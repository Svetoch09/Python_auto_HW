...
class TestPositiveCRUD:
    ...
    def test_create_and_get_project(self, project_service: ProjectService,
                                    new_project_id):
    ...
    def test_update_project_title(self, project_service: ProjectService,
                                  new_project_id):
    ...
    def test_delete_project(self, project_service: ProjectService,
                            new_project_id):
        ...
        assert project_service.response_json.get("deleted") is True, \
            "Ожидалось, что поле 'deleted' будет true после удаления."
...
class TestSchemaValidation:
    ...
    def test_schema_after_creation(self, project_service: ProjectService):
        ...
        project_service.check_response_schema(expected_fields=('id',))
        # запятая, чтобы Python понял, что это кортеж
        # из одного элемента.

    def test_schema_after_get_by_id(self, project_service: ProjectService,
                                    new_project_id):
        project_service.get_project_by_id(new_project_id)
