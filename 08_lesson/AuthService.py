...
class AuthService:
    ...
    def check_response_status(self, expected_status=200):  # по умолчанию ждет 200
    ...
    def get_company_id(self):
        self.response = requests.post(self.base_url + '/auth/companies', json=self.creds)
    ...
    def create_key(self, company_id):
        ...
        self.response = requests.post(self.base_url + '/auth/keys', json=key_payload)
    ...
    def get_keys_list(self, company_id):
        key_payload = self._get_key_payload(company_id)
        self.response = requests.post(self.base_url + '/auth/keys/get', json=key_payload)
        self.check_response_status()
        self.response_json = self.response.json()
    ...
    def delete_key(self, key_value):
        self.response = requests.delete(self.base_url + f'/auth/keys/{key_value}')
        self.check_response_status()
