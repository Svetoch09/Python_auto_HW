...
class ProjectService:
    ...
    def get_projects_list(self):
        self.response = requests.get(self.base_url + '/projects',
                                     headers=self.headers)
        self.response_json = self.response.json()
        return self.response_json

    def create_project(self, project_data):
        self.response = requests.post(self.base_url + '/projects',
                                      headers=self.headers, json=project_data)
        self.response_json = self.response.json()
        return self.response_json

    def update_project(self, project_id, project_data):
        self.response = requests.put(self.base_url + f'/projects/{project_id}',
                                     headers=self.headers, json=project_data)
        self.response_json = self.response.json()
        return self.response_json
    ...
    def get_project_by_id(self, project_id):
        self.response = requests.get(self.base_url + f'/projects/{project_id}',
                                     headers=self.headers)
        self.response_json = self.response.json()
        return self.response_json
    ...
    def check_response_schema(self, expected_fields=('deleted', 'id', 'title')):
        ...
        if 'id' in response_data:
            assert isinstance(response_data['id'], str), \
                (f"Ошибка схемы: 'id' должен быть строкой, "
                 f"получен {type(response_data['id'])}")

        if 'title' in response_data:
            assert isinstance(response_data['title'], str), \
                (f"Ошибка схемы: 'title' должен быть строкой,"
                 f" получен {type(response_data['title'])}")
